...
def apply_theme(theme, window):
    ...
    for button_name in button_names:
        if hasattr(window, button_name):
            ...
            icon_path = iconpath(f"{button_name.split('_', 2)[2]}.ico")
    ...
    if hasattr(window, 'audio_player_widget'):
        ...
        if hasattr(audio_player, 'play_button'):
            audio_player.play_button.setIcon(QIcon(iconpath("play.ico")))
        if hasattr(audio_player, 'stop_button'):
            audio_player.stop_button.setIcon(QIcon(iconpath("stop.ico")))
        if hasattr(audio_player, 'mute_button'):
            if audio_player.audio_output.isMuted():
                audio_player.mute_button.setIcon(QIcon(iconpath("mute.ico")))
            else:
                audio_player.mute_button.setIcon(QIcon(iconpath("vol_high.ico")))
