...
class SettingsManager:
    def __init__(self):
        ...
        self.default_settings = {
            "SETTINGS/SaveProgramSettings": True,
            ...
            "EDITOR/FontSize": 12,
            "EDITOR/Wrapped": True,
            "EDITOR/ConfirmTextModified": False,
...
#     def __init__(self, text, parent=None, v_offset=-1): # Make offset configurable
#         super().__init__(text, parent)
#         self.vertical_offset = v_offset # Negative value moves text up

#     def paintEvent(self, event):
#         painter = QPainter(self)
#         # Don't translate the painter itself

#         # Get the original drawing rectangle
#         draw_rect = self.contentsRect() # Use contentsRect to respect potential borders/margins

#         # Adjust the vertical position of the drawing rectangle
#         draw_rect.translate(0, self.vertical_offset)

#         # Draw the text within the adjusted rectangle, using existing alignment
#         # Use existing flags (like AlignLeft | AlignVCenter set outside)
#         # The painter will clip automatically if text goes outside widget bounds
#         painter.drawText(draw_rect, int(self.alignment()), self.text())

#         # Don't call super().paintEvent(event) as we've handled drawing


# """
# I came back and tried again and again. I couldn't sleep.
# It's an issue with the way the statusbar is generated inside PyQt, no matter what you do it keeps 2 pixels 
# between the top of the statusbar and the top of whatever widget you add inside. 
# It would rather clip the border of the widget than allow it to go further up.
# So I managed to push the text slightly higher from the styleSheet and removed the onHover background and it looks ok.

# I wasted too much time on this, I guess I'll have to open an issue with the PyQt6 team.
# """
