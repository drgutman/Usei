...
class SearchDialog(QDialog):
    ...
    def __init__(self, settings_manager, parent, state='collapsed'):
        ...
        self.up_button.setProperty("tooltip", "Previous match")
        ...
        self.down_button.setProperty("tooltip", "Next match")
        ...
        self.close_button.setProperty("tooltip", "Close")
    ...
    def init_ui(self):
        # Clear layout before setting new state
        while self.main_layout.count():
            item = self.main_layout.takeAt(0)
            widget = item.widget()
            if widget:
                widget.setParent(None)  # This avoids deleting the widget completely

        # Create a horizontal layout for the button and fields
        self.horizontal_layout = QHBoxLayout()
        self.horizontal_layout.setContentsMargins(0, 0, 0, 0)
        self.horizontal_layout.setSpacing(4)  # Add some spacing between the button and fields
        self.horizontal_layout.setAlignment(Qt.AlignmentFlag.AlignLeft)  # Align horizontally to the left

        # Add the button to the left side
        self.horizontal_layout.addWidget(self.toggle_replace)

        # Create a horizontal layout for the search field and buttons
        self.search_layout = QHBoxLayout()
        self.search_layout.setContentsMargins(0, 0, 0, 2)
        self.search_layout.setSpacing(0)  # Add some spacing between search field and buttons
        self.search_layout.addWidget(self.search_field)

        # Add the up_button, down_button, match_case_button, and close_button next to the search field
        ...
        # Create a vertical layout for the replace field (it stays below search field)
        self.fields_layout = QVBoxLayout()
        self.fields_layout.setContentsMargins(2, 0, 0, 2)
        self.fields_layout.setSpacing(0)
        self.fields_layout.setAlignment(Qt.AlignmentFlag.AlignLeft)  # Align vertically to the left

        # Add the replace field based on the state
        if self.state == 'collapsed':
            self.setFixedHeight(36)
            self.toggle_replace.setFixedSize(24, 36)
            self.fields_layout.addLayout(self.search_layout)  # Add the horizontal search layout
            ...
            self.toggle_replace.setFixedSize(24, 66)
            self.fields_layout.addLayout(self.search_layout)  # Add the horizontal search layout

            # Create replace_layout only if expanded
            self.replace_layout = QHBoxLayout()  # New replace_layout instance
            self.replace_layout.setContentsMargins(0, 2, 0, 2)
            ...
            self.fields_layout.addLayout(self.replace_layout)
            self.search_field.show()
            self.replace_field.show()
            self.replace_button.show()
            self.replace_all_button.show()

        # Add the fields layout to the horizontal layout
    ...
    def toggle_state(self, new_state):
        if self.state != new_state:
            ...
            self.init_ui()  # Refresh the UI based on the new state
        self.update_icons()
    ...
    def update_icons(self):
        ...
        if self.state == 'expanded':
            toggle_icon_path = 'res/light/down.ico' if theme == 'Light' else 'res/dark/down.ico'
        else:
            toggle_icon_path = 'res/light/right.ico' if theme == 'Light' else 'res/dark/right.ico'
        self.toggle_replace.setIcon(QIcon(toggle_icon_path))

        # Update up and down button icons
        up_icon_path = 'res/light/arrow_up.ico' if theme == 'Light' else 'res/dark/arrow_up.ico'
        down_icon_path = 'res/light/arrow_down.ico' if theme == 'Light' else 'res/dark/arrow_down.ico'
        close_icon_path = 'res/light/close.ico' if theme == 'Light' else 'res/dark/close.ico'
        case_icon_path = 'res/light/case.ico' if theme == 'Light' else 'res/dark/case.ico'
        replace_icon_path = 'res/light/swap.ico' if theme == 'Light' else 'res/dark/swap.ico'
        replace_all_icon_path = 'res/light/swap_all.ico' if theme == 'Light' else 'res/dark/swap_all.ico'
        ...
        self.match_case_button.setIcon(QIcon(case_icon_path))
        self.replace_button.setIcon(QIcon(replace_icon_path))
        self.replace_all_button.setIcon(QIcon(replace_all_icon_path))
...
def show_search_dialog(main_window, state, selection=None):
    ...
    if dialog.isVisible():
        ...
        dialog.show()
        dialog.toggle_state(state)
        dialog.search_field.setFocus()  # Move focus to the search field
