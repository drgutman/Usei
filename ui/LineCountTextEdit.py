...
from core.utils import settings_manager
...
class LineCountTextEdit(QTextEdit):
    ...
    def update_text_stats(self):
        # --- Calculate current line count directly ---
        current_line_count = 0
        block = self.document().begin()
        while block.isValid():
            # Use block.layout().lineCount() which respects wrapping
            current_line_count += block.layout().lineCount()
            block = block.next()
        if current_line_count < 1:
            current_line_count = 1 # Ensure at least 1 line is reported
        # --- End of line count calculation ---

        longest_line_index = 0
        longest_line_chars = 0
        block = self.document().begin()
        line_index = 0 # Use line_index for the *block* number, not visual line
        while block.isValid():
            block_text = block.text()
            if len(block_text) > longest_line_chars:
                longest_line_chars = len(block_text)
                longest_line_index = line_index # Report the block index
            line_index += 1
            block = block.next()

        # Emit the *calculated* line count and longest line info
        global_signals.text_stats_signal.emit(f" lines: {current_line_count} - longest block: {longest_line_index+1} with {longest_line_chars} chars.")
