...
import os
import sys
from core.utils import SettingsManager, CustomSlider
...
class AudioPlayerWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.play_button.setIcon(QIcon(os.path.join(self.res_folder, 'empty.ico')))
        self.play_button.clicked.connect(self.toggle_play_pause)

        self.stop_button = QPushButton()
        self.stop_button.setFixedSize(26, 26)
        self.stop_button.setIcon(QIcon(os.path.join(self.res_folder, 'empty.ico')))
        ...
        self.mute_button.setIcon(QIcon(os.path.join(self.res_folder, 'empty.ico')))  
    ...
    def load_file(self, file_path, stop_before_loading=True):
        ...
        if not os.path.exists(file_path):
            global_signals.output_signal.emit(f"Warning: File does not exist: {file_path}")
            return
    ...
    def add_to_playlist(self, file_path):
        if not os.path.exists(file_path):
            global_signals.output_signal.emit(f"Warning: Cannot add non-existent file to playlist: {file_path}")
            return

        self.playlist_files.append(file_path)
        global_signals.output_signal.emit(f"Added to playlist: {file_path}")
    ...
    def load_current_file(self):
        if self.current_index < len(self.playlist_files) and os.path.exists(self.playlist_files[self.current_index]):
            ...
            global_signals.output_signal.emit(f"Loading file #{self.current_index}: {current_file}")
            if self.autoplay_checkbox.isChecked():
                self.player.play()
    ...
    def handle_fused_file_update(self, success, message, temp_folder):
        ...
        if isinstance(message, str) and os.path.exists(message):
            ...
            global_signals.output_signal.emit(f"Fusion failed: {message}")
    ...
    def on_media_status_changed(self, status):
        if status == QMediaPlayer.MediaStatus.EndOfMedia:
            ...
            global_signals.output_signal.emit(f"End of media for file #{self.current_index}")
    ...
    def cleanup_temp_files(self, temp_folder):
        if temp_folder and os.path.exists(temp_folder):
            try:
                ...
                global_signals.output_signal.emit(f"Cleaned up temporary folder: {temp_folder}")
            except Exception as e:
                global_signals.output_signal.emit(f"Cleanup error: {str(e)}")
    ...
    def on_playlist_finished(self):
        ...
        if self.fused_file_finished and os.path.exists(self.fused_file_path):
            if self.player.source().toLocalFile() != self.fused_file_path:
                global_signals.output_signal.emit(f"Loading fused file: {self.fused_file_path}")
    ...
    def on_playback_state_changed(self, state):
        ...
        self.play_button.setIcon(QIcon(os.path.join(self.res_folder, theme, icon)))
    ...
    def toggle_mute(self):
        ...
        if self.audio_output.isMuted():
            ...
            self.mute_button.setIcon(QIcon(os.path.join(self.res_folder, theme, 'vol_high.ico')))
        else:
            self.audio_output.setMuted(True)
            self.mute_button.setIcon(QIcon(os.path.join(self.res_folder, theme, 'mute.ico')))
