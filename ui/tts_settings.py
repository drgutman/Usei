...
from core.utils import SettingsManager, OverlayWidget
from core.signals import global_signals
from core.tts_render import render_text
...
class TTSPropertiesWindow(QWidget):
    ...
    def __init__(self, tts_editor, audio_player):
        ...
        renderOutputLayout.setContentsMargins(0, 4, 0, 8)
        ...
        movie = QMovie(f"{self.res_folder}/wait_.gif")
    ...
    def renderButtonClicked(self):
        ...
        if missing_params:
            ...
            if not output_dir: # Handle cases where only a filename is given (current dir)
                output_dir = "."
            if not os.path.isdir(output_dir):
                QMessageBox.warning(
                    self,
                    "Invalid Output Path",
                    f"The directory '{output_dir}' for the output file does not exist. Please choose a valid output file path."
                )
                return

            # --- Add filename validation ---
            filename = os.path.basename(output_file)
            # Basic invalid characters for Windows/common systems
            invalid_chars = r'[<>:"/\\|?*]'
            if re.search(invalid_chars, filename):
                 QMessageBox.warning(
                    self,
                    "Invalid Filename",
                    f"The filename '{filename}' contains invalid characters ({invalid_chars}). Please choose a different name."
                )
                 return
            ...
            if os.path.exists(output_file):
                # ... (rest of the overwrite/save new/cancel logic remains the same) ...
                msg_box = QMessageBox(self)
                msg_box.setWindowTitle("File Exists")
                msg_box.setText(f"The file '{os.path.basename(output_file)}' already exists. What would you like to do?") # Show only filename
                ...
                if msg_box.clickedButton() == cancel_button:
                    ...
                    def get_unique_filename(file_path):
                        # ... (get_unique_filename logic remains the same) ...
                    ...
                    self.outputFileEditBox.setText(output_file) # Update the edit box with the new name

            # Now proceed with rendering
            # ... (call render_text as before) ...
